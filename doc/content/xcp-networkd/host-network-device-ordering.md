...
Particularly, when this assumption holds, a consistent initial order can be
...
for the network devices on multiple hosts. However, this only works if the
assumption of identical hardware, firmware, software, and network device
placement holds. When this assumption doesn't hold, the order can be generated
from a user's configuration. However, administrators would prefer to avoid this
as much as possible when managing many hosts.
...
The current unordered network device is considered newly added.
